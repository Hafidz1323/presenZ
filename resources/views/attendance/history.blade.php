...
@section('content')
    <div class="space-y-6">
        <div class="bg-white rounded-3xl border border-slate-200/50 shadow-sm p-6 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
            <div>
                <h2 class="font-black text-2xl text-slate-900 tracking-tight">Riwayat Absensi Saya</h2>
                <p class="text-xs text-slate-500 mt-0.5">Tinjau log kehadiran harian, koordinat lokasi GPS, dan dokumentasi foto absen Anda.</p>
            </div>
        </div>
                ...
                <table class="w-full text-sm text-left text-slate-600">
                    <thead class="text-xs font-bold text-slate-700 uppercase bg-slate-50 border-b border-slate-200/40">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-4">Dokumentasi Foto</th>
                        </tr>
                    </thead>
                        ...
                        @foreach ($attendances as $att)
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                        ...
                                        @if($att->check_in_lat)
                                            <div class="flex items-center gap-1 mt-1">
                                                <a 
                                                    ...
                                                    title="Lihat peta koordinat check-in"
                                                >
                                            ...
                                            @if($att->check_out_lat)
                                                <div class="flex items-center gap-1 mt-1">
                                                    <a 
                                                        ...
                                                        title="Lihat peta koordinat check-out"
                                                    >
                                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path>
                                                        </svg>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span 
                                        class="px-2.5 py-1 text-xxs rounded-full font-black uppercase tracking-wider border {{ $att->status === 'Hadir' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-rose-50 text-rose-700 border-rose-200' }}"
                                    >
                                        {{ $att->status }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex gap-2">
                                        ...
                                        @if($att->check_in_photo)
                                            <div class="relative group cursor-pointer" onclick="openLightbox('{{ asset('storage/' . $att->check_in_photo) }}', 'Foto Check In Anda', '{{ Carbon\Carbon::parse($att->check_in_time)->isoFormat('D MMMM YYYY, H:i') }}', '{{ $att->check_in_lat }}', '{{ $att->check_in_long }}', '{{ $att->check_in_address }}')">
                                                <img 
                                        ...
                                        @if($att->check_out_photo)
                                            <div class="relative group cursor-pointer" onclick="openLightbox('{{ asset('storage/' . $att->check_out_photo) }}', 'Foto Check Out Anda', '{{ Carbon\Carbon::parse($att->check_out_time)->isoFormat('D MMMM YYYY, H:i') }}', '{{ $att->check_out_lat }}', '{{ $att->check_out_long }}', '{{ $att->check_out_address }}')">
                                                <img 
                        ...
                        @if(count($attendances) === 0)
                            <tr>
                                <td colspan="6" class="px-6 py-8 text-center text-slate-400 font-semibold">
                                    Belum ada data riwayat absensi Anda.
                                </td>
                            </tr>
                        @endif
                    </tbody>
    ...
    <div id="lightbox-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-sm transition duration-300 animate-in fade-in">
        <div class="bg-white rounded-3xl max-w-md w-full overflow-hidden shadow-2xl border border-slate-200/60 relative animate-in fade-in zoom-in-95 duration-200">
            ...
            <!-- Info Details -->
            <div class="p-6">
                <h3 class="text-lg font-black text-slate-900 leading-snug">Dokumentasi Absensi Anda</h3>
                <p class="text-xs text-slate-500 font-medium mt-1">Waktu terekam: <span class="font-semibold text-slate-800" id="modal-time">-</span></p>
                ...
                <div id="modal-gps-details" class="mt-4 pt-4 border-t border-slate-100 space-y-3">
                    ...
                    <div class="flex items-center justify-between gap-4">
                        <div>
                            <span class="block text-slate-400 font-bold uppercase tracking-wider text-[9px]">Koordinat Posisi</span>
    ...
    <!-- Lightbox Script -->
    <script>
        function openLightbox(url, title, time, lat, lng, address) {
